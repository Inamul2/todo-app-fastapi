...
@app.get("/completed")
async def home(request: Request, db: Session = Depends(get_db)):
    todos = db.query(models.Todo).where(models.Todo.completed==True).order_by(models.Todo.id.desc())
    return templates.TemplateResponse("index.html", {"request": request, "todos": todos})

@app.get("/pending")
async def home(request: Request, db: Session = Depends(get_db)):
    todos = db.query(models.Todo).where(models.Todo.completed==False).order_by(models.Todo.id.desc())
    return templates.TemplateResponse("index.html", {"request": request, "todos": todos})
...
async def add(request: Request, db: Session = Depends(get_db)):
    todo = db.query(models.Todo).delete()
    db.commit()
    return RedirectResponse(url=app.url_path_for("home"), status_code=status.HTTP_303_SEE_OTHER)